                        ...
                        <div class="card p-3 mb-3">
                            <div class="row">
                                <div class="img col-sm-12  col-md-4">
                                    <img class="img logo-img" src="{{ $event['logo'] }}" alt="">
                                </div>
                                <div class="content col-md-8 col-sm-12">
                                    <h2 class="heading text-center"> {{ $event['event_name'] }} </h2>
                                    <p class="event-summary"> {{ $event['event_description'] }} </p>
                                    <div class="d-flex justify-content-center">
                                        <table class="table table-sm table-borderless text-left">
                                            <tr>
                                                <td><a href="/event-info?id={{ $event['id'] }}" class="btn btn-outline-info">Read More</a></td>
                                                @if( $event['enrollment'] == 1 )
                                                    <td><a href="/enroll?id={{ $event['id'] }}" class="btn btn-outline-success">Enroll</a></td>
                                                @elseif( $event['enrollment'] == 0 )
